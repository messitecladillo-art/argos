...
def _format_user_task(content: str, leader_id: str) -> str:
    return (
        ...
        "4. 创建 worker 子任务后，必须立即调用 kanban_complete(summary=...) 关闭当前父 Kanban 任务；这个 complete 只表示“调度阶段已完成”，不是用户最终答复。\n"
        "5. 收到汇总任务时，只基于任务正文里的 worker 结果输出最终总结，不要重复派发同一批任务。\n"
        "6. 只有任务不需要 worker 时，才直接调用 kanban_complete 完成当前 Kanban 任务，然后再给用户最终答复。\n"
        "7. 严禁使用内置 kanban_create / kanban_comment / kanban_assign 创建或模拟 worker 子任务；这些任务不会写入团队总线，也不会出现在 Web 团队看板里。\n"
        "8. 如果你已经用 kanban_create 创建了依赖任务，必须改为调用 mcp_agent_bus_create_kanban_worker_tasks；不要把内置 Kanban 任务当作 worker 派发结果。\n"
