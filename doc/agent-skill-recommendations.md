...
本文记录当前团队 agent 的推荐 skills。安装方式按项目现有 Skills 管理实现填写：使用 Git 仓库地址作为 `source_url`，使用 skill 目录作为 `subdir`。
...
当前安装接口同步执行，成功后直接返回 `skill`，不会返回 `job_id`。

```bash
curl -X POST http://127.0.0.1:5050/api/agents/<agent_id>/skills/install \
