...
const argosDebug = window.localStorage?.getItem("argosDebug") !== "0";
...
function checkArgosStatus() {
  if (!argosStatusPromise) {
    argosStatusPromise = fetch("/api/argos/status")
      .then(async (response) => {
        const data = await response.json().catch(() => ({}));
        if (!response.ok || !data.ok) argosStatusPromise = null;
        return { response, data };
      })
      .catch((error) => {
        argosStatusPromise = null;
        throw error;
      });
  }
  return argosStatusPromise;
}
const soulState = {
  agentId: "",
...
const kanbanPanelsStorageKey = "argosKanbanPanelsExpanded";
...
function debugLog(event, payload) {
  if (!argosDebug) return;
  console.log(`[argos-debug] ${event}`, payload);
}
...
function playNotificationSound(kind, agentId) {
  if (window.localStorage?.getItem("argosSound") === "0") return;
  const now = Date.now();
...
function logTerminalPayload(event, session, payload) {
  ...
  if (summary.hasReplacementChar || summary.suspiciousControlCount > 0) {
    console.warn(`[argos-terminal] suspicious ${event}`, {
      agentId: session.agentId,
      type: payload?.type,
      ...summary,
    });
...
function refreshModelConfigSelects() {
  ...
  if (createAgentModelConfig) {
    createAgentModelConfig.innerHTML = `<option value="">继承当前 active profile 配置</option>${options}`;
  }
  if (agentModelSelect) {
...
async function exportTeamArchive() {
  ...
  try {
    ...
    const filename = match ? decodeURIComponent(match[1].replace(/"/g, "")) : "argos.zip";
...
function setSelectedAgent(agentId, agentName, force = false, options = {}) {
  ...
  renderInteractions();
  if (terminalTitle) terminalTitle.textContent = agentId ? `${name} · Argos Terminal` : "Agent Terminal";
  const session = ensureTerminalSession(agentId);
...
transferImportFile?.addEventListener("change", () => {
  transferLastInspectedFile = null;
  if (transferFileName) {
    transferFileName.textContent = transferImportFile.files?.[0]?.name || "选择 argos-时间.zip 文件";
  }
  if (transferImportSubmit) transferImportSubmit.disabled = true;
  if (transferImportPreview) transferImportPreview.hidden = true;
  setTransferStatus(transferImportStatus, "");
...
async function ensureArgosReadyForAgentCreation(button) {
  if (button) button.disabled = true;
  try {
    const { response, data } = await checkArgosStatus();
...
if (openCreateAgent) {
  openCreateAgent.addEventListener("click", async () => {
    if (await ensureArgosReadyForAgentCreation(openCreateAgent)) openModal();
  });
  window.setTimeout(() => {
    void checkArgosStatus().catch(() => {});
  }, 300);
}
