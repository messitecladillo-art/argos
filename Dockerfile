# ── Hermes Agent Team — production Dockerfile ──────────────────
# Build:  docker build -t hermes-agent-team .
# Run:    docker run -p 5050:5050 -v ./data:/app/data --env-file .env hermes-agent-team
#
...
LABEL org.opencontainers.image.title="Hermes Agent Team"
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD curl -sf http://127.0.0.1:5050/api/hermes/status || exit 1
...
CMD ["uvicorn", "app.asgi:create_asgi_app", "--factory", \
